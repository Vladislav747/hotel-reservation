...
func (h *RoomHandler) HandleBookRoom(c *fiber.Ctx) error {
	...
	where := bson.M{"fromData": bson.M{"$gte": params.FromDate}, "tillDate": bson.M{"$lte": params.TillDate}}
	...
	if !ok {
		return c.Status(http.StatusBadRequest).JSON(genericResp{Type: "error", Msg: fmt.Sprintf("room %s already booked", roomID.String())})
	}
...
func (h *RoomHandler) isRoomAvailableForBooking(ctx context.Context, roomID primitive.ObjectID, params BookRoomParams) (bool, error) {
	where := bson.M{"roomID": roomID, "fromData": bson.M{"$gte": params.FromDate}, "tillDate": bson.M{"$lte": params.TillDate}}
